...
describe("ActionForm", () => {
  ...
  it("shows 'New Action' heading in create mode", () => {
    render(<ActionForm config={mockConfig} onSave={onSave} onCancel={onCancel} />);
    expect(screen.getByText("New Action")).toBeInTheDocument();
  });

  it("shows 'Edit Action' heading in edit mode", () => {
    render(
      <ActionForm config={mockConfig} initial={mockAction} onSave={onSave} onCancel={onCancel} />
    );
  ...
  it("pre-fills user prompt field from initial prop", () => {
    ...
    expect(screen.getByDisplayValue("Improve clarity and grammar")).toBeInTheDocument();
  });
  ...
  it("renders provider options from config", () => {
    render(<ActionForm config={mockConfig} onSave={onSave} onCancel={onCancel} />);
    expect(screen.getByText("Anthropic Claude (anthropic)")).toBeInTheDocument();
  });
  ...
  it("calls onSave with trimmed name and prompt", async () => {
    ...
    await user.type(screen.getByPlaceholderText("e.g. Refine wording"), "  My Action  ");
    await user.type(
    ...
    await waitFor(() =>
      expect(onSave).toHaveBeenCalledWith(
        expect.objectContaining({ name: "My Action", userPrompt: "Do the thing" })
      )
    );
  });
  ...
  it("omits model field from payload when model input is blank", async () => {
    ...
    await user.type(screen.getByPlaceholderText("e.g. Refine wording"), "Action");
